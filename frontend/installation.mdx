...
Once you have the necessary files, execute the following commands to clean the environment and start the deployment:
...
**How to Add Entries:**
- **Linux & macOS**: Edit the file using `sudo nano /etc/hosts`, add the entries, save, and exit.
- **Windows**: Open `C:\Windows\System32\drivers\etc\hosts` in Notepad as Administrator, add the entries, and save the file.
...
Go to a Google Chrome browser (prefered) and type `http://mirasys_frnt.local` to access the frontend system.