...
Download `install.sh` [script](https://github.com/microsoft/terminal) file. Put the file in `~/Documents` folder, run following commands and it will install all the necessary libraries, docker dependencies, make folders and give necessary permissions
```bash install_fe.sh
...
<Note>
   running the `install_fe.sh`, then running commands of this section is not required if the `install_fe.sh` installation went successfully. In case of failure, you can run the following commands separately.
</Note>
