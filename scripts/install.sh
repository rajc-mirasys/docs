...
# FFmpeg install with Nvidia
cd $HOME/Downloads
git clone https://git.videolan.org/git/ffmpeg/nv-codec-headers.git
cd nv-codec-headers && sudo make install && cd –
wget https://ffmpeg.org/releases/ffmpeg-7.1.1.tar.xz && tar -xf ffmpeg-7.1.1.tar.xz && cd ffmpeg-7.1.1
...
uv pip install -U onnx onnxruntime onnxslim requests tomli rich
